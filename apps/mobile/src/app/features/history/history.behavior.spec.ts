...
function setup() {
  ...
  const gateway = { dataHostId: () => activeHost().id, online: signal(true), historySessions: signal([session]), history: vi.fn().mockResolvedValue([session]), historyMessages: vi.fn().mockResolvedValue([message]) };
...
describe('History transcript', () => {
  it('returns to its safe browse origin and offers the session identity and fresh project action', async () => {
    const { router } = setup();
    TestBed.overrideComponent(HistoryDetailComponent, { set: { imports: [], schemas: [NO_ERRORS_SCHEMA] } });
    const fixture = TestBed.createComponent(HistoryDetailComponent);
    (fixture.componentInstance as unknown as { chatId: () => string }).chatId = signal('archive-a');
    fixture.detectChanges(); await fixture.whenStable(); fixture.detectChanges();
