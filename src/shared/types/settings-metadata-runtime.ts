...
export const RUNTIME_SETTINGS_METADATA: SettingMetadata[] = [
  ...
  {
    key: 'contextSpendRecoveryMinOccupancyPercent',
    label: 'Minimum context fill for spend-based compaction (%)',
    description: 'Harness interrupts and compacts an agent after a long run of work (4× its context window in total tokens since its last compaction), but only once its context is at least this full. Compacting a mostly empty context throws detail away and saves little. 0 compacts at any fill.',
    type: 'number',
    category: 'memory',
    min: 0,
    max: 100,
  ...
  {
    key: 'contextSpendRecoveryBackstopMultiple',
    label: 'Always compact after spend (× context window)',
    description: 'Interrupt and compact an agent once it has used this many context windows of tokens since its last compaction, however full its context is. This caps runaway turns. Must be at least 4.',
    type: 'number',
    category: 'memory',
    min: 4,
    max: 1000,
  ...
  {
    key: 'localAiGuardDefaultFallbackPolicy',
    label: 'Local AI paid-fallback policy',
    description: 'What to do when an enrolled local model cannot handle a routed helper call.',
    type: 'select',
    options: [
      { value: 'allow-silently', label: 'Allow silently' },
      { value: 'notify-and-allow', label: 'Notify and allow' },
      { value: 'require-confirmation', label: 'Require confirmation' },
      { value: 'defer-locally', label: 'Defer locally' },
      { value: 'block-paid-fallback', label: 'Block paid fallback' },
    ],
    category: 'advanced',
  },
  {
    key: 'localAiGuardDailyFallbackBudgetUsd',
    label: 'Local AI daily fallback budget (USD)',
    description: 'Hard daily ceiling for paid Local AI fallback dispatches. Leave empty for no ceiling.',
    type: 'number',
    category: 'advanced',
    min: 0,
    max: 1_000_000,
  ...
  {
    key: 'localAiGuardConfirmAboveInputTokens',
    label: 'Confirm Local AI fallback above input tokens',
    description: 'Require confirmation above this estimated input-token count. Leave empty to disable.',
    type: 'number',
    category: 'advanced',
    min: 0,
    max: 100_000_000,
