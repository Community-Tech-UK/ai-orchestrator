...
Answer:`;
  ...
  private async generateCompletion(systemPrompt: string, userPrompt: string): Promise<string> {
    ...
    if (this.config.provider === 'anthropic' || this.config.anthropicApiKey) {
      ...
      if (!breaker.isAllowed()) {
        ...
        try {
          const result = await generateWithAnthropic(this.config, this.providerAvailability(), systemPrompt, userPrompt);
    ...
    if (this.config.provider === 'ollama' || this.ollamaAvailable !== false) {
      ...
      if (!breaker.isAllowed()) {
        ...
        const startTime = Date.now();
        try {
          const result = await generateWithOllama(this.config, this.providerAvailability(), systemPrompt, userPrompt);
          breaker.recordSuccess(Date.now() - startTime);
          return result;
        } catch (error) {
          breaker.recordFailure(error as Error, Date.now() - startTime);
    ...
    if (this.config.provider === 'openai' || this.config.openaiApiKey) {
      ...
      if (!breaker.isAllowed()) {
        ...
        const startTime = Date.now();
        try {
          const result = await generateWithOpenAI(this.config, this.providerAvailability(), systemPrompt, userPrompt);
          breaker.recordSuccess(Date.now() - startTime);
          return result;
        } catch (error) {
          breaker.recordFailure(error as Error, Date.now() - startTime);
    ...
    try {
      ...
      if (this.config.provider === 'anthropic' || this.config.anthropicApiKey) {
        ...
        if (!breaker.isAllowed()) {
          ...
          const startTime = Date.now();
          try {
            yield* streamWithAnthropic(this.config, this.providerAvailability(), systemPrompt, userPrompt, requestId, controller.signal);
            breaker.recordSuccess(Date.now() - startTime);
            return;
          } catch (error) {
      ...
      if (this.config.provider === 'ollama' || this.ollamaAvailable !== false) {
        ...
        if (!breaker.isAllowed()) {
          ...
          const startTime = Date.now();
          try {
            yield* streamWithOllama(this.config, this.providerAvailability(), systemPrompt, userPrompt, requestId, controller.signal);
            breaker.recordSuccess(Date.now() - startTime);
            return;
          } catch (error) {
            if ((error as Error).name === 'AbortError') throw error;
      ...
      if (this.config.provider === 'openai' || this.config.openaiApiKey) {
        ...
        if (!breaker.isAllowed()) {
          ...
          const startTime = Date.now();
          try {
            yield* streamWithOpenAI(this.config, this.providerAvailability(), systemPrompt, userPrompt, requestId, controller.signal);
            breaker.recordSuccess(Date.now() - startTime);
            return;
          } catch (error) {
            if ((error as Error).name === 'AbortError') throw error;
  ...
  private providerAvailability(): LlmProviderAvailability {
    ...
    return {
      ...
      set anthropicAvailable(value) { self.anthropicAvailable = value; },
      get ollamaAvailable() { return self.ollamaAvailable; },
      set ollamaAvailable(value) { self.ollamaAvailable = value; },
      get openaiAvailable() { return self.openaiAvailable; },
      set openaiAvailable(value) { self.openaiAvailable = value; },
    };
  }
