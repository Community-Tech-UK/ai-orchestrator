...
describe('browser extension assets', () => {
  it('keeps existing-tab access as a one-shot user-gesture capture', () => {
    ...
    expect(background).not.toContain('tabs.onUpdated');
    expect(background).not.toContain('tabs.onRemoved');
    expect(background).not.toContain("type: 'poll_commands'");
    expect(background).not.toContain("type: 'complete_command'");
    expect(background).not.toContain('sharedTargets');
