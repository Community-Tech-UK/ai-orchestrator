...
function createOperationChild(
  ...
  // `--import` is resolved by Node's ESM loader, which on Windows rejects a bare
  // absolute path (`C:\...`) with ERR_UNSUPPORTED_ESM_URL_SCHEME. Pass a file://
  // URL so the tsx loader resolves cross-platform. (This .ts branch only runs
  // outside Electron in dev/tests; production forks the compiled .js child.)
  const execArgv = entrypoint.endsWith('.ts')
    ? ['--import', pathToFileURL(createRequire(__filename).resolve('tsx')).href]
    : [];
