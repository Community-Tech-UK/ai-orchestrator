...
const TOOL_GUIDANCE: Partial<Record<DesktopMcpToolName, string>> = {
  ...
  'computer.click': 'Use a fresh computer.accessibility_snapshot token. Target an elementUid or coordinates inside the observed approved app window; candidates with inputEligible: false cannot be clicked. Whether a sensitive control (sign-in, submit, payment) can be clicked depends on the Computer Use autonomy level: blocked at `guarded`, permitted at `trusted` and `unrestricted`.',
  'computer.type_text': 'Use a fresh accessibility snapshot and an elementUid or observed focused field. At the `guarded` autonomy level, password and other secure fields are blocked. At `trusted` and `unrestricted` they are permitted; typed text is redacted from the audit log, but still prefer a vault or credential-fill route over typing a secret directly.',
  'computer.hotkey': 'The approved app must remain active. Which combinations are refused depends on the Computer Use autonomy level: at `guarded`, Enter/Return/Space plus destructive, quit and system-level shortcuts; at `trusted`, only the destructive, quit and system-level ones; at `unrestricted`, none.',
