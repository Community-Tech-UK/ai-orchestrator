...
export function registerRuntimePluginHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.RUNTIME_PLUGINS_LIST, async (): Promise<IpcResponse> => {
    try {
      return { success: true, data: await packageManager.list() };
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_event: IpcMainInvokeEvent, payload: unknown): Promise<IpcResponse> => {
      try {
        ...
        return { success: true, data: await packageManager.install(validated.source) };
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_event: IpcMainInvokeEvent, payload: unknown): Promise<IpcResponse> => {
      try {
        ...
        return {
          success: true,
          data: await packageManager.update(validated.pluginId, validated.source),
        };
      } catch (error) {
