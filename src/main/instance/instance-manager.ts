...
import { inheritSubagentPermissions } from './instance-permissions-facade';
import { InstancePermissionRequestFlow } from './instance-permission-request-flow';
import { cleanupAdjudicatorBreakerForInstance } from '../security/approval-adjudicator';
...
export class InstanceManager extends EventEmitter {
  ...
  private readonly permissionRequests = new InstancePermissionRequestFlow({
    getInstance: (id) => this.getInstance(id),
    sendInputResponse: (id, response, permissionKey) => this.sendInputResponse(id, response, permissionKey),
    resumeAfterDeferredPermission: (id, approved) => this.resumeAfterDeferredPermission(id, approved),
    addToOutputBuffer: (inst, msg) => this.communication.addToOutputBuffer(inst, msg),
    publishOutput: (id, msg) => this.publishOutput(id, msg),
  ...
  constructor(
    ...
    getInstanceProviderLimitHandler().configure({
      ...
      getThreadIdentifiers: (id) => {
        const inst = this.state.getInstance(id);
        return inst ? { historyThreadId: inst.historyThreadId, sessionId: inst.sessionId } : null;
      },
      getProviderModel: (id) => {
        const inst = this.state.getInstance(id);
    ...
    this.orchestrationMgr.setupOrchestrationHandlers(
      {
        ...
        maxSpawnDepth: settingsAll.maxSpawnDepth,
      },
      ...
      this.orchestrationMgr.setupOrchestrationHandlers(
        {
          ...
          maxSpawnDepth: newSettings.maxSpawnDepth,
        },
        (inst, msg) => this.communication.addToOutputBuffer(inst, msg),
        (instanceId, message) => this.publishOutput(instanceId, message),
      );
  ...
  recordInputRequiredPermissionDecision(params: {
    ...
    this.permissionRequests.recordUserDecision(params);
  }

  clearPendingInputRequiredPermission(instanceId: string, requestId: string): void {
    this.permissionRequests.clearPending(instanceId, requestId);
  }

  private clearPendingInteractiveState(instanceId: string): void {
    this.permissionRequests.clearInstance(instanceId);
  }
  ...
  setAllowIncomingSessionMessages(instanceId: string, allow: boolean): void {
    return this.lifecycle.setAllowIncomingSessionMessages(instanceId, allow);
  }
    ...
    if (options?.crossSessionSourceId) {
      userMessage.metadata = {
        ...
        crossSessionMessage: {
          ...
          hopCount: options.crossSessionHopCount ?? 0,
        },
      };
    }
