...
export interface InputCapRecoveryOps {
  ...
  send(): Promise<void>;
  /** Start Codex compaction; resolves false when it is unavailable. */
  compact(): Promise<boolean>;
  /** Wait (bounded) for a started compaction to land. */
  awaitCompaction(): Promise<void>;
 ...
 *
 *   1. Compact + wait for it to land + retry once — preserves context via
