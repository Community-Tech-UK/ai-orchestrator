...
export type { LoopStartupSnapshot } from './loop-stage-machine.types';
...
- IMPLEMENT done **and the plan or completion inventory is fully implemented & verified** →
    ...
    const freshEyesReviewBlock = config.completion.crossModelReview?.enabled
      ? `\n\n## Fresh-Eyes Review Gate\nFresh-eyes review is enabled: when you declare done, the coordinator runs an independent cross-model review (it reuses the previous clean verdict only when git reports no change since the state that verdict was issued against, so re-declaring without changing anything gains you nothing). Any ${config.completion.crossModelReview.blockingSeverities.join('/')} severity finding is automatically injected as a user intervention here in the prompt, and the loop continues with you addressing it. If you address every intervention and the reviewer has no further blocking findings, the loop accepts completion.\n`
      : '';
