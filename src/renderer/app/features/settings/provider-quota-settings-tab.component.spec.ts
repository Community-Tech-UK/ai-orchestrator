...
const styles = readFileSync(
  resolve(specDirectory, './provider-quota-settings-tab.component.scss'),
  'utf8',
);

await resolveComponentResources((url) => {
  if (url.endsWith('provider-quota-settings-tab.component.scss')) {
  ...
  describe('lifecycle', () => {
    ...
    it('seeds intervals from readPollIntervals on init', () => {
      store.readPollIntervals.mockReturnValue({
        claude: 15 * 60 * 1000, codex: 0, gemini: 0, antigravity: 0, copilot: 60 * 60 * 1000, cursor: 0, grok: 0, opencode: 0,
      });
      fixture.detectChanges();
