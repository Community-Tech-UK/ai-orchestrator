...
async function main(): Promise<void> {
  assertNode22Plus();
  const bundle = path.resolve('dist/aio-mcp-cli/index.js');
  if (!fs.existsSync(bundle)) {
    throw new Error(`Missing ${bundle} — run npm run build:aio-mcp-cli first`);
  }
  const outDir = path.resolve('dist/aio-mcp-cli-sea');
