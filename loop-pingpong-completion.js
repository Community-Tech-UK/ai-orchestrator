...
exports.evaluatePingPongCompletion = evaluatePingPongCompletion;
const logger_1 = require("../logging/logger");
const loop_types_1 = require("../../shared/types/loop.types");
const loop_diff_1 = require("./loop-diff");
const loop_cwd_1 = require("./loop-cwd");
const content_egress_gate_1 = require("../security/content-egress-gate");
const loop_coordinator_completion_gates_1 = require("./loop-coordinator-completion-gates");
const loop_coordinator_utils_1 = require("./loop-coordinator-utils");
const pingpong_intent_classifier_1 = require("./pingpong-intent-classifier");
const loop_pingpong_builder_done_1 = require("./loop-pingpong-builder-done");
const agentic_pingpong_reviewer_1 = require("./agentic-pingpong-reviewer");
const loop_fresh_eyes_reviewer_1 = require("./loop-fresh-eyes-reviewer");
const abortable_promise_settlement_1 = require("./abortable-promise-settlement");
const logger = (0, logger_1.getLogger)('LoopPingPong');
/**
...
function applyLedgerUpdates(ledger, classifications, findings, round) {
...
function buildIntervention(review, blocking, advisory, localAdvisory) {
    const head = `Ping-pong reviewer (${review.reviewerProvider}) did NOT approve. ` +
        `${review.summary}\n`;
    const blockingBlock = blocking.length > 0
        ? `\nBLOCKING — you must fix or rebut each before re-declaring done:\n` +
            blocking
                .map((f, i) => `${i + 1}. [${f.severity.toUpperCase()}] ${f.title}${f.file ? ` (${f.file})` : ''}\n` +
                `   evidence: ${f.evidence}\n   ${f.body}`)
                .join('\n')
        : '';
...
function buildLocalAdvisoryBlock(localAdvisory) {
    return localAdvisory.length > 0
        ? `\n\nLocal-model advisory evidence (visible for consideration, but not blocking unless a remote reviewer independently finds it):\n` +
            localAdvisory.map((finding, index) => `${index + 1}. [${finding.severity}] ${finding.title}${finding.file ? ` (${finding.file})` : ''}\n` +
                `   ${finding.body}`).join('\n')
        : '';
}
function localAdvisoryFailure(error) {
    const reason = error instanceof Error ? error.message : String(error);
    return { status: 'failed', findings: [], summary: reason, reason };
}
function emitLocalAdvisoryStatus(emit, state, seq, stage, local) {
...
function costCapExceeded(state) {
    const caps = state.config.caps;
    if (caps.maxCostCents !== null && state.totalCostCents >= caps.maxCostCents)
        return true;
    if (caps.maxTokens !== null && state.totalTokens >= caps.maxTokens)
        return true;
    return false;
}
/**
 ...
 */
async function evaluatePingPongCompletion(deps) {
    ...
    if (!reviewCfg || !ppCfg?.enabled)
        return null;
    const pp = (state.pingPong ??= (0, loop_types_1.defaultPingPongState)());
    const maxRounds = (0, loop_types_1.clampPingPongMaxRounds)(ppCfg.maxRounds);
    ...
    const builderVerdict = await (0, loop_pingpong_builder_done_1.resolvePingPongBuilderDone)(iteration.completionSignalsFired, classifyCleanReview, {
        goal: state.config.initialPrompt,
        // Work product, not loop state — see `loop-cwd.ts`.
        executionCwd: (0, loop_cwd_1.loopExecutionCwd)(state.config),
    ...
    if (costCapExceeded(state)) {
        return {
            ...
            reason: `Ping-pong stopped on cost cap after ${pp.roundCount} round(s) ` +
    ...
    if (pp.roundCount >= maxRounds) {
        return {
            status: 'cap-reached',
            reason: `Ping-pong hit its ${maxRounds}-round cap without mutual convergence. ` +
                `Open issues:\n${summarizeOpen(pp.ledger)}`,
        };
    }
    ...
    const subject = deps.resolveSubject
        ? await deps.resolveSubject(state, fullOutput).catch(() => (0, pingpong_intent_classifier_1.resolvePingPongSubject)(state, fullOutput))
        : (0, pingpong_intent_classifier_1.resolvePingPongSubject)(state, fullOutput);
    pp.subject = subject;
    const productionChanges = iteration.filesChanged.filter((f) => (0, loop_coordinator_completion_gates_1.isReviewDrivenProductionChange)(f.path));
    ...
    const diffCwd = (0, loop_cwd_1.loopExecutionCwd)(state.config);
    const workspaceDiff = (0, loop_diff_1.collectWorkspaceDiff)(diffCwd);
    ...
    const diffEgress = (0, content_egress_gate_1.redactForEgress)(workspaceDiff.diff, { kind: 'diff' });
    ...
    const reviewer = deps.reviewer ?? agentic_pingpong_reviewer_1.agenticPingPongReviewer;
    const localAdvisoryReviewer = deps.localAdvisoryReviewer ?? loop_fresh_eyes_reviewer_1.runLocalOnlyFreshEyesReview;
    ...
    const timeoutMs = (0, loop_types_1.clampPingPongReviewerTimeoutSeconds)(ppCfg.reviewerTimeoutSeconds) * 1000;
    (0, loop_pingpong_builder_done_1.emitBuilderDoneSignalActivity)(emit, { loopRunId: state.id, seq, stage, verdict: builderVerdict });
    ...
    try {
        const remoteSettlement = (0, abortable_promise_settlement_1.settlePromise)(() => reviewer(reviewerInput));
        const localSettlement = (0, abortable_promise_settlement_1.settlePromise)(() => localAdvisoryReviewer(localInput));
        ...
        if (remoteCancelled) {
            ...
            const localResult = await (0, abortable_promise_settlement_1.waitForSettlementOrAbort)(localSettlement, deps.signal);
    ...
    const advisoryFindings = localAdvisory.status === 'used'
        ? localAdvisory.findings.map((finding) => ({ ...finding, advisory: true }))
        : [];
    ...
    if (review.reviewerProvider)
        pp.lastReviewerProvider = review.reviewerProvider;
    ...
    if (review.verdict === 'UNRELIABLE') {
        const fault = review.fault ?? 'malformed_output';
        const availability = (0, loop_types_1.isReviewerAvailabilityFault)(fault);
        ...
        if (pp.consecutiveUnreliableRounds >= threshold) {
            ...
            return availability
                ? {
                    ...
                    reason: `Could not obtain a ping-pong review: the reviewer provider was ` +
                ...
                : {
                    ...
                    reason: `Ping-pong reviewer produced UNUSABLE output ${pp.consecutiveUnreliableRounds} rounds ` +
    ...
    if (review.verdict === 'APPROVED') {
        ...
        if (subject === 'impl' && deps.runVerify) {
            const verify = await deps.runVerify().catch((error) => ({
                ok: false,
                output: error instanceof Error ? error.message : String(error),
            }));
            if (!verify.ok) {
                state.pendingInterventions.push((0, loop_types_1.createLoopPendingInput)('The ping-pong reviewer APPROVED, but the configured verify command FAILED. ' +
                    'Treat this as a blocking issue and fix it before re-declaring done:\n\n' +
                    ((0, loop_coordinator_utils_1.excerptVerifyOutput)(verify.output, 8192) || '(verify produced no output)') +
        ...
        return {
            status: 'completed',
            reason: `Ping-pong converged after ${round} round(s): reviewer (${review.reviewerProvider}) ` +
    ...
    state.pendingInterventions.push((0, loop_types_1.createLoopPendingInput)(buildIntervention(review, blocking, advisory, advisoryFindings)));
    if (blocking.length === 0) {
        ...
        if (pp.lowOnlyChurnRounds >= MAX_LOW_ONLY_ROUNDS) {
            return {
                ...
                reason: `Ping-pong converged-or-arbitrated after ${round} rounds: only low-severity ` +
    ...
    if (persistedBlocking.length > 0 && noProductionChangeThisRound) {
        ...
        if (pp.builderUnaddressedRounds >= MAX_UNADDRESSED_ROUNDS) {
            return {
                ...
                reason: `Builder declared done ${pp.builderUnaddressedRounds} rounds running without ` +
        ...
        if (pp.consecutiveContradictoryRounds >= MAX_CONTRADICTORY_ROUNDS) {
            return {
                status: 'needs-human-arbitration',
                reason: `Builder and reviewer deadlocked over the same issue(s) for ` +
...
function summarizeOpen(ledger) {
    const open = ledger.filter((i) => i.status === 'open' || i.status === 'regression');
    if (open.length === 0)
        return '(no open issues recorded)';
    return open
        .map((i) => `- [${i.severity}] ${i.title}${i.file ? ` (${i.file})` : ''} — ${i.status}`)
        .join('\n');
}
//# sourceMappingURL=loop-pingpong-completion.js.map