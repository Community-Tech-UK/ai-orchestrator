...
| The packaged app suddenly reports `localExtension: {state: "not_installed"}` mid-campaign | A batch's dev app took the machine's Chrome native-messaging manifest. It lives at `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.ai_orchestrator.browser_gateway.json`, is **machine-global**, and `AIO_DEV_USER_DATA_PATH` never isolated it — so the last app to start owns the local extension channel and the other one reports itself uninstalled with remediation text that wrongly blames the extension. Filed and fixed as LT-520 (packaged always wins; an unpackaged app declines unless `AIO_CLAIM_LOCAL_BROWSER_MANIFEST=1`). **Confirmed live 2026-08-25 and no longer a hazard:** the packaged app now claims the manifest at start-up, and seven dev apps launched afterwards all declined it, logging `Another Harness install owns the Chrome native-messaging manifest — leaving it alone`. You no longer need to restore the manifest, and deleting a `/tmp/aio-lt-*` profile can no longer break the user's local channel. If you *do* still see this symptom, check the manifest's mtime against your dev app's start time before assuming the fix regressed — and note a dev app deliberately has no local extension channel of its own, so `browser.health` from a dev app reporting no local extension is expected, not a fault. Remote (`windows-pc`) browser work goes over the worker relay and is unaffected. |
...
- [ ] Remove `/tmp/aio-lt-*` workspaces and `_scratch/` fixtures. (Since LT-520 was confirmed live on 2026-08-25, this no longer risks the Chrome native-messaging manifest — dev apps decline to claim it.)
