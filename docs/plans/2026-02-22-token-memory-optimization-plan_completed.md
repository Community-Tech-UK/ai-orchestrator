...
> | Task | State | Evidence |
> |---|---|---|
> | 1 — Observation masking in smart compaction | **BUILT, NOT REACHED IN PRODUCTION** | **Correction 2026-09-23:** `maskStaleToolOutputs` (`src/main/rlm/smart-compaction.ts:706`) is called from `performCompaction` (`:463`), whose only entry is `checkAndCompact` (`:261`). Nothing outside that file calls `checkAndCompact`; `memory-bootstrap.ts:73` only constructs the singleton. The "live compaction path" claim was wrong — this code never runs in the app. Not re-scoped here. |
...
> | 6 — Early consensus | **BUILT 2026-09-23 (opt-in, default off), retargeted** | See as-built note below. |
> | 7 — Debate defense skipping | **BUILT 2026-09-23 (opt-in, default off), retargeted** | See as-built note below. |
> | 8–9 — Event-based decision logs | **BUILT 2026-09-23 (opt-in, default off), retargeted** | See as-built note below. |
...
> | 13 — Final integration verification | **RUN 2026-09-23** | The canonical gate set (both typechecks, lint, LOC ratchet, `build:main`, `build:renderer`, full suite) ran as part of the outstanding-plans sweep, followed by an independent fresh-eyes review. |
>
> **Decision:** the shipped tasks (2, 11, 12) are done and need no further work. Task 1's code
> exists but has no production caller (see the 2026-09-23 correction above). Task 2's
...
>
> **Closed 2026-09-23 (outstanding-plans sweep).** Tasks 6–9 and 13 are done (as-built below).
> Tasks 3–5 and 10 stay superseded as decided above: T10 shipped, and T7 ended in
> `2026-09-05-loop-child-transcript-prune_spec_completed.md`, which recommended not building it.
> Task 1's code has no production caller because the whole RLM `checkAndCompact` path is unused;
> whether to wire that path in is a separate decision, recorded in the sweep plan, not unfinished
> work on this plan.
>
> **AS-BUILT 2026-09-23 (tasks 6–9, W7).** Each was aimed at the code path that actually runs
> today instead of the path this plan named. All three features are opt-in, and with them off
> the existing behaviour is unchanged.
>
> - **Task 6, early consensus.** The plan targeted `MultiVerifyCoordinator`, but nothing reachable
>   from the UI calls it. The Verification dashboard goes `VERIFICATION_START_CLI` →
>   `CliVerificationCoordinator`, so that is where this was built. The logic is in
>   `src/main/orchestration/verification-early-consensus.ts`: `EarlyTerminationConfig`,
>   `DEFAULT_EARLY_TERMINATION` (off, 0.8 threshold, at least 2 agents), `checkEarlyConsensus`
>   (mean pairwise cosine over the orchestration TF-IDF embeddings; errored answers are ignored,
>   and the shared Key Points / Overall Confidence / Reasoning Summary sections are removed
>   first so they cannot fake agreement), `raceWithEarlyConsensus` and `dropPendingAgents`. It is
>   wired in at `cli-verification-extension.ts:365`. Agents that are still running get
>   `terminate(false)`. `session.cancelled` is never set, so this is not treated as a user
>   cancel. Their wait loops exit (`:528`), and an `idle` that arrives after the drop is ignored
>   (`:505`). They are marked `error: 'dropped: early consensus'`, so the analysis skips them,
>   and `verification:early-consensus` is emitted. A dropped agent's partial token and cost
>   spend is still counted in the totals (for an agent with no streamed output, the prompt is
>   estimated). No "tokens saved" figure is reported anywhere. Providers whose `sendMessage`
>   blocks for the whole turn (Codex exec, Gemini) reject when the drop terminates them; that
>   rejection, and a failing second `terminate()`, are treated as the drop rather than as an
>   agent error, so the agent keeps the drop label and its spend (fixed after the fresh-eyes
>   review; covered by the blocking-provider spec).
>   `VerificationConfig.earlyTermination` and `VerificationResult.earlyConsensus` were added.
>   The Zod field is at `provider.schemas.ts:243`, and the IPC handler passes it through at
>   `cli-verification-ipc-handler.ts:461`. There is no UI toggle and no forwarding of the
>   renderer event yet. Specs: `verification-early-consensus.spec.ts`,
>   `cli-verification-extension.early-consensus.spec.ts`.
> - **Task 7, skipping the defense round.** The convergence check that already existed in
>   `runDebate` can never skip defense, because the critique round copies each agent's previous
>   position and so its score always equals the initial round's. A different signal is used
>   instead: `DebateConfig.skipDefenseOnLowSeverityCritiques` (and its Zod field at
>   `orchestration.schemas.ts:51`), plus `critiquesShowNoMaterialIssues()` in
>   `debate-consensus.ts`. That returns true only when every agent filed critiques and every
>   one of them is `low`. At `debate-coordinator.ts:217` the defense round is skipped, with a
>   log line and a `debate:defense-skipped` event, and the debate goes straight to synthesis.
>   This does not happen when a user intervention was just applied. Spec:
>   `__tests__/debate-defense-skip.spec.ts`.
> - **Tasks 8–9, the decision log.** This targets restart-with-summary compaction
>   (`src/main/app/compaction-runtime.ts` `restartCompact`) instead of the RLM
>   `SessionCompactor`, which is a dead path. It has no LLM extraction step. Built so far:
>   `src/shared/types/observation-event.types.ts` (keyed by `instanceId`, with an optional
>   `compactionMarkerId`); `src/main/context/observation-extractor.ts` (uppercase
>   DECISION/ERROR/BUG/ARCHITECTURE/IMPORTANT tags, error messages at priority 1, tool results
>   at priority 3 capped at 150 chars, todo/next markers as priority-3 `task_progress`,
>   secrets redacted, de-duplicated, capped at 30); RLM migration
>   `066_session_observation_events` (`rlm-migrations-066-070.ts`, registered in
>   `rlm-schema.ts`); the `rlm-observation-events.ts` store; and
>   `src/main/context/compaction-decision-log.ts`. With the new setting
>   `compactionDecisionLogEnabled` off (the default) it returns `[]` and touches nothing. With
>   it on, it records the events (a DB failure only logs a warning) and returns a
>   `Decision log:` block wrapped in `<decision_log>`, with at most 10 entries of priority 2 or
>   higher. The setting is registered in the settings types, defaults, surfacing, metadata
>   (memory tab) and control policy. The block is spliced into `restartCompact`'s continuity
>   prompt right after "Unresolved items" (`compaction-runtime.ts`). Specs:
>   `observation-extractor.spec.ts`, `compaction-decision-log.spec.ts`,
>   `rlm-observation-events.spec.ts`, and `compaction-runtime.spec.ts` (setting on: block present
>   and rows recorded; off: no block and nothing recorded; recorder failure: the restart still
>   completes).
