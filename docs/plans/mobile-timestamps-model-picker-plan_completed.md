...
> **Reconciled 2026-09-23 (outstanding-plans sweep, [plan](2026-09-23-outstanding-plans-sweep_plan_completed.md)): COMPLETE.** The header below still says "PLANNED (not started)", which is stale. Both features shipped: timestamp stamps via `buildDisplayItems()`/`formatStampLabel()` in `apps/mobile/src/app/shared/transcript-items.ts` rendered by the conversation view, `MobileModelDto` in `mobile-gateway.types.ts`, `GET /api/models` and `POST /api/instances/:id/model` in `mobile-gateway-model-handlers.ts`, and `model-sheet.component.ts`.
