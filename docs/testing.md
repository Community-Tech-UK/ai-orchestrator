...
- **A full run inside a `.worktrees/*` checkout fails `scripts/__tests__/dependency-compatibility.spec.ts`, and that is the environment, not the code.** Those worktrees get a `node_modules` of per-entry symlinks into the root checkout's install, so `npm ls --omit=dev --all` resolves every package outside the project and reports the whole tree as `extraneous`. Measured 2026-09-20 on an otherwise clean queue worktree: 1 of 25745 tests failed there, and the same spec passed 4/4 in the root checkout at the same commit. Re-run that one spec from the root checkout before believing it; nothing about a symlinked tree tells you whether the shipped dependency tree is valid.
