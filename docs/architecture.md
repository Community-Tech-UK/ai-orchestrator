...
- **Provider event fidelity** — `adapter-runtime-event-bridge.ts` maps the shared adapter event vocabulary through a pure mapper before envelope publication. Every normalized event with adapter provenance carries a JSON-safe optional `raw` payload; `ProviderEventCaptureService` batches those events to the conversation ledger's dedicated `provider_event_captures` table without requiring a chat thread. Capture is always on, retained for 30 days by jitter-scheduled maintenance, and can be exported as redacted JSONL fixture/golden pairs with `tsx scripts/capture-provider-fixture.ts --db <conversation-ledger.db> --instance <id> --scenario <name>`.
