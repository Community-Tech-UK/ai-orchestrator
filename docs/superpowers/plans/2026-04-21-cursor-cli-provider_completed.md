...
> **Reconciled 2026-09-23 (outstanding-plans sweep, [plan](../../plans/2026-09-23-outstanding-plans-sweep_plan_completed.md)): COMPLETE.** `cursor-cli-adapter.ts`, `cursor-cli-provider.ts` with `CURSOR_CAPABILITIES`, registry wiring and `agentId` in the create payloads. Cursor later moved to the shared ACP adapter.
