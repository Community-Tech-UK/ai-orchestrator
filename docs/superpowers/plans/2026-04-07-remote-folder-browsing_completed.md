...
> **Reconciled 2026-09-23 (outstanding-plans sweep, [plan](../../plans/2026-09-23-outstanding-plans-sweep_plan_completed.md)): COMPLETE.** Remote FS types and RPC, `node-filesystem-handler.ts` routed from `rpc-event-router.ts`, `filesystem-service.ts`, `remote-browse-modal.component.ts`; later extended with directory sync and file transfer.
