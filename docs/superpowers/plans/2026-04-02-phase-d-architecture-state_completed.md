...
> **Reconciled 2026-09-23 (outstanding-plans sweep, [plan](../../plans/2026-09-23-outstanding-plans-sweep_plan_completed.md)): COMPLETE.** The immutable store, selectors and observers exist and the store is wired (`getAppStore` in `instance-event-forwarding.ts` and `event-bus/state-sync-snapshot.ts`); CoreDeps narrow DI is passed to `InstanceLifecycleManager`.
