...
> **Reconciled 2026-09-23 (outstanding-plans sweep, [plan](../../plans/2026-09-23-outstanding-plans-sweep_plan_completed.md)): COMPLETE.** Implemented without its own plan: `CODEX_TIMEOUTS` (per-RPC timeouts, 90s notification idle, 3s graceful shutdown), `detached: true` process groups (`app-server-broker.ts`) and the stdin writable guard (`app-server-client.ts`).
